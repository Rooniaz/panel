...
import { faDownload, faExclamationCircle, faThumbsUp, faClock } from '@fortawesome/free-solid-svg-icons';
...
const InstallModalComponent = ({ pluginId, pluginName, provider, onInstalled, open, onClose }: Props) => {
    const uuid = ServerContext.useStoreState((state) => state.server.data!.uuid);
    const { clearFlashes, clearAndAddHttpError } = useFlashKey('plugins:install-modal');
    ...
    const safePluginName = pluginName || 'Plugin';
    const safeProvider = provider || 'modrinth';
    ...
    useEffect(() => {
        ...
        try {
            http.get(`/api/client/servers/${uuid}/plugins/${safePluginId}/versions`, {
                params: { provider: safeProvider },
            })
                .then(({ data }) => {
            ...
            setError('An unexpected error occurred while loading plugin versions.');
            setLoading(false);
        }
    }, [pluginId, open]);
    ...
    const submit = ({ version }: Values, { setSubmitting }: FormikHelpers<Values>) => {
        ...
        console.log('Installing plugin:', { pluginId: safePluginId, version: safeVersion, provider: safeProvider });

        setSubmitting(true);

        try {
            installPlugin(uuid, safePluginId, safeVersion, safeProvider)
                .then(() => {
            ...
            setError('An unexpected error occurred while installing the plugin.');
            setSubmitting(false);
        }
    };

    return (
        <Dialog open={open && !isClosing} onClose={handleClose} title={`Install ${pluginName}`} hideCloseIcon={false}>
            <ModalContent isClosing={isClosing}>
                <FlashMessageRender byKey={'plugins:install-modal'} css={tw`mb-4`} />
                ...
                {loading ? (
                    <LoadingContainer>
                        <Spinner size={'large'} />
                        <LoadingText>Loading plugin versions...</LoadingText>
                    </LoadingContainer>
                ) : versions.length === 0 ? (
                    <Alert type='warning' className='my-4'>
                        <div css={tw`flex flex-col items-center w-full py-4`}>
                            ...
                            <p>No versions available for this plugin.</p>
                        </div>
                    </Alert>
                ) : (
                    <>
                            ...
                            <PluginHeader>
                                {project.icon_url ? (
                                    <PluginIcon
                                        ...
                                        alt={project.name || project.title || pluginName}
                                    />
                                ) : (
                                    <PlaceholderIcon>
                                ...
                                <PluginInfo>
                                    <PluginTitle>{project.name || project.title || pluginName}</PluginTitle>
                                    <PluginDescription>
                                        {project.description ||
