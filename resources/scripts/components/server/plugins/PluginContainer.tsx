...
export default () => {
    ...
    const loadGameVersions = () => {
        http.get(`/api/client/servers/${uuid}/plugins/minecraft-versions`, {
            ...
            .then(({ data }) => {
                ...
                setMinecraftVersions([{ value: '', label: 'All Versions' }, ...versions]);
            })
            .catch((error) => {
    ...
    const loadPluginLoaders = () => {
        ...
        http.get(`/api/client/servers/${uuid}/plugins/loaders`, {
            ...
            .then(({ data }) => {
                ...
                setPluginLoaders([{ value: '', label: 'All Loaders' }, ...loaders]);
            })
            .catch((error) => {
    ...
    return (
        <ServerContentBlock title={'Minecraft Plugins'} showFlashKey={'plugins'} css={tw`flex flex-col`}>
            ...
            {filters.provider === 'spigotmc' || filters.provider === 'curseforge' ? (
                <FilterContainer css={tw`grid-cols-1 md:grid-cols-2 lg:grid-cols-5`}>
                    ...
                    <FilterGroup>
                        ...
                        >
                            {minecraftVersions && minecraftVersions.length > 0 ? (
                    ...
                    <FilterGroup css={tw`col-span-1 md:col-span-2 lg:col-span-1`}>
                        <StyledInput value={searchTerm} onChange={handleSearch} placeholder='Search plugins...' />
                    </FilterGroup>
                </FilterContainer>
            ) : (
                <FilterContainer>
                        ...
                        <div css={tw`grid gap-4 md:grid-cols-2 lg:grid-cols-3`}>
                            {items.length > 0 ? (
                                items.map((plugin: Plugin) => (
                                    <PluginCard key={plugin.id} onClick={() => handleInstall(plugin.id, plugin.name)}>
                                        ...
                                        <PluginHeader>
                                            ...
                                            <PluginInfo>
                                                <h3 css={tw`text-sm font-semibold truncate mb-1`}>{plugin.name}</h3>
                                                {plugin.author && plugin.author.trim() !== '' && (
                                                    <p css={tw`text-xs text-neutral-300 mb-1`}>By {plugin.author}</p>
                                                )}
                                                <PluginDescription>{plugin.short_description}</PluginDescription>
                                            </PluginInfo>
                                        </PluginHeader>
                                        <PluginFooter>
                                            <PluginStats>
                                ...
                                <div css={tw`col-span-full`}>
                                    <Alert type='warning'>No plugins found matching your criteria.</Alert>
                                </div>
                            )}
                        </div>
                    )}
