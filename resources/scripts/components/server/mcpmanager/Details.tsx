        ...
        const submit = (values: { target: string; item: string; quantity: string }, { setSubmitting }: any) => {
            ...
            giveItem(
                ...
                .then(() => {
                    ...
                    close();
                    // ✅ Refresh player items after giving item (เพิ่ม delay เพื่อให้ server process command)
                    if (onRefreshPlayerItems) {
                        // Wait a bit for server to process the command (เพิ่มเป็น 2 วินาที)
                        console.log('[GiveItem] Waiting for server to process command...');
                        setTimeout(() => {
                            if (onRefreshPlayerItems) {
                                console.log('[GiveItem] Refreshing player inventory...');
                                onRefreshPlayerItems();
                            }
                        }, 2000); // ✅ เพิ่มจาก 1500ms เป็น 2000ms เพื่อให้ server process command เสร็จ
                    }
                })
                .catch((error) => {
                    setSubmitting(false);
    ...
    const tabs = [
        {
            ...
            content: (
                <PlayerInventory
                    ...
                    onRefreshPlayerItems={onRefreshPlayerItems}
                    serverVersion={fastQueryData?.info?.version?.name}
