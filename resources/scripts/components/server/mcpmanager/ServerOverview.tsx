    ...
    return (
        <TitledGreyBox title='Server Overview' css={tw`mb-6`}>
            <div css={tw`p-4 space-y-4`}>
                ...
                <div css={tw`bg-neutral-900 p-3 rounded-lg text-center text-sm`} dangerouslySetInnerHTML={motdHtml} />
                <div css={tw`grid grid-cols-2 gap-4`}>
                    <Stat
                        label='Players'
                        value={`${players?.online?.length || 0} / ${players?.max || 'N/A'}`}
                        icon={undefined}
                    />
                    <Stat label='Version' value={info?.version?.name || 'N/A'} icon={undefined} />
                </div>
            </div>
        </TitledGreyBox>
    );
