...
const StatItem: React.FC<{ name: string; value: string | number; type: 'item' | 'mob' | 'custom' }> = ({
    name,
    value,
    type,
}) => (
...
const StatBoard: React.FC<{ title: string; icon: any; stats: StatEntry[]; type: 'item' | 'mob' | 'custom' }> = ({
    title,
    icon,
    stats,
    type,
...
export const PlayerStatsBoards: React.FC<PlayerStatsBoardsProps> = ({ stats, isLoading }) => {
    ...
    const processStats = (
        category: 'used' | 'dropped' | 'mined' | 'killed' | 'killed_by' | 'picked_up',
        limit = 20
    ): StatEntry[] => {
