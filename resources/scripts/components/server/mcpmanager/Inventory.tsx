...
export default function PlayerInventory({
    ...
    // ✅ แสดง error เฉพาะเมื่อมี error message จริงๆ (ไม่ใช่แค่ไม่มี data)
    if (!playerItems) {
        // กำลัง loading → แสดง loading state
        return (
            <div css={tw`bg-neutral-800 rounded-lg border border-neutral-700 shadow-md`}>
                <div css={tw`p-4 border-b border-neutral-700`}>
                    <div css={tw`flex items-center justify-between`}>
    ...
    // ✅ แสดง inventory ตามปกติเสมอ (แม้จะว่างเปล่า)
    // ไม่แสดง error message เพื่อให้ผู้ใช้เห็น inventory grid แม้จะไม่มี items
    // ถ้ามี error แต่เป็น normal case (404, 500, not found) จะแสดง empty inventory แทน
    ...
    const RemoveItemDialog = asDialog({ title: 'Remove Item' })(() => {
        ...
        const handleRemoveItem = async () => {
            ...
            try {
                ...
                // Refresh player items after removing item (only once)
                if (onRefreshPlayerItems) {
                    // Wait a bit for server to process the command
                    setTimeout(() => {
                        if (onRefreshPlayerItems) {
                            onRefreshPlayerItems();
                        }
                ...
                <div css={tw`flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 md:gap-0`}>
                    ...
                    <div css={tw`flex items-center gap-2`}>
                        <button
                            onClick={() => {
                                if (onRefreshPlayerItems) {
                                    console.log('[PlayerItems] Manual refresh triggered');
                                    // ✅ Refresh ทันที และ refresh อีกครั้งหลังจาก 1 วินาที เพื่อให้ server save player data
                                    onRefreshPlayerItems();
                                    setTimeout(() => {
                                        if (onRefreshPlayerItems) {
                                            console.log(
                            ...
                            css={tw`flex items-center gap-1 md:gap-2 px-3 py-1.5 rounded-lg transition-colors text-xs md:text-sm border shadow-sm bg-gray-900 border-green-400 text-green-300 hover:bg-gray-800 hover:border-green-300`}
                            title='Refresh inventory (will refresh twice to ensure latest data)'
                        >
                            <FontAwesomeIcon icon={faSync} css={tw`w-4 h-4`} />
