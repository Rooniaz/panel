...
import getMinecraftForks, {
    ForkInfo,
    MinecraftForkResponse,
} from '@/api/server/minecraftversionchanger/getMinecraftForks';
...
import getCurrentMinecraftVersion, {
    CurrentMinecraftVersionResponse,
} from '@/api/server/minecraftversionchanger/getCurrentMinecraftVersion';
...
export default () => {
    ...
    useEffect(() => {
        ...
        Promise.all([getMinecraftForks(uuid), getCurrentMinecraftVersion(uuid)])
                            ...
                            <CurrentVersionInfo>
                                ...
                                <CurrentVersionText>
                                    ...
                                    <p css={tw`text-sm text-gray-400`}>
                                        <span css={tw`text-gray-50 font-semibold`}>{currentVersion.type}</span>{' '}
                                        {currentVersion.version} (build:{' '}
                                        <span css={tw`text-blue-400`}>{currentVersion.build}</span>)
                                    </p>
                                </CurrentVersionText>
                            </CurrentVersionInfo>
                        </CurrentVersionCard>
                    ...
                    <div css={tw`mb-8`}>
                        <CategoryContainer>
                            {Object.entries(forks).map(([type, info]) => (
                                <ForkCard key={type} onClick={() => handleSelectFork(type, info)}>
                                    <ForkHeader>
                                        <ForkIcon
