...
import getMinecraftVersions, {
    VersionInfo,
    BuildInfo,
} from '@/api/server/minecraftversionchanger/getMinecraftVersions';
...
export default ({ visible, forkType, forkInfo, onDismissed, onVersionChanged }: Props) => {
    ...
    const filteredAndSortedVersions = Object.entries(versions)
        .filter(([version, versionInfo]) => {
            ...
            if (!/^\d+\.\d+(\.\d+)?(?:[-_]?(?:pre|rc|beta|alpha|snapshot|experimental|dev)[-_]?\d*)?$/i.test(version))
                return false;
            ...
            .then((data) => {
                ...
                const filteredAndSortedKeys = Object.entries(data.versions)
                    .filter(([version, versionInfo]) => {
                        ...
                        // Only filter out versions that don't match the basic version pattern
                        if (
                            !/^\d+\.\d+(\.\d+)?(?:[-_]?(?:pre|rc|beta|alpha|snapshot|experimental|dev)[-_]?\d*)?$/i.test(
                                version
                            )
                        )
                            return false;
                                ...
                                >
                                    {builds.map((build) => {
                                        const isSpecialType = [
                                            ...
                                            'sponge',
                                            'legacyfabric',
                                        ].includes(forkType.toLowerCase());
                                        const buildValue = isSpecialType
                                            ? build.name.toString()
