...
class SettingsController extends ClientApiController
{
    ...
    {
        // Get external_id for Spring Boot API call
        // Spring Boot endpoint: DELETE /api/servers/external/{externalId}
        // Header: X-Pterodactyl-Key: {api_key}
        ...
        if ($externalId) {
            try {
                ...
                $apiKey = config('services.spring_boot.api_key', '');
                ...
                // Add X-Pterodactyl-Key header if API key is configured
                if ($apiKey) {
                    $headers['X-Pterodactyl-Key'] = $apiKey;
                }

                $endpoint = "{$springBootUrl}/api/servers/external/{$externalId}";
            ...
            Log::warning('No external_id found for server, skipping Spring Boot deletion', [
                'server_uuid' => $server->uuid,
                'server_uuid_short' => $server->uuidShort,
                'server_id' => $server->id,
            ]);
