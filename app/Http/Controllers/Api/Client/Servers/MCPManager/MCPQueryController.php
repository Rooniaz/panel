...
class MCPQueryController extends Controller
{
    ...
    public function getPlayerItems(Request $request, Server $server, string $uuid): JsonResponse
    {
        try {
            ...
            // Note: getPlayerItems จะเรียก forceSavePlayerData อัตโนมัติอยู่แล้ว
            // แต่เรียกที่นี่อีกครั้งเพื่อให้แน่ใจว่า save แล้วก่อนอ่านไฟล์
            
            $data = $this->mcpActionService->getPlayerItems($server, $uuid, $world);
            
            // ✅ Check if service returned an error (service returns array with 'error' key instead of throwing)
            if (isset($data['error'])) {
                // Return 200 with error in response body (not 500) so frontend can handle it
                return new JsonResponse([
                    'error' => $data['error'],
                    'inventory' => $data['inventory'] ?? [],
                    'ender_chest' => $data['ender_chest'] ?? [],
                    'armor' => $data['armor'] ?? [],
            ...
            Log::error('Failed to get player items', [
                'server' => $server->uuid,
                'player_uuid' => $uuid,
                'world' => $request->query('world', 'world'),
                'error' => $e->getMessage(),
            ...
            // Return 200 with error (not 500) so frontend can handle it gracefully
            return new JsonResponse([
                'error' => 'Failed to get player items: ' . $e->getMessage(),
                'inventory' => [],
                'ender_chest' => [],
                'armor' => [],
                'offhand' => [],
    ...
    {
        try {
            Log::info('MCPQueryController: Getting detected worlds', [
                'server' => $server->uuid,
            ]);
            
            $worlds = $this->mcpActionService->getDetectedWorlds($server);
            
            Log::info('MCPQueryController: Detected worlds result', [
                'server' => $server->uuid,
            ...
            // Log the exception for debugging
            Log::error('Failed to get detected worlds', [
                'server' => $server->uuid,
                'error' => $e->getMessage(),
