...
class MCPActionService
{
    ...
    public function giveItem(Server $server, string $uuid, string $player, string $item, int $amount = 1, int $metadata = 0, ?int $slot = null, ?string $type = null): void
    {
        ...
        if ($slot !== null && $type !== null) {
            $slotId = '';
            if ($type === 'inventory') {
                // ✅ Bottom inventory (hotbar) slots: 0-8
                // Main inventory slots: 9-35
                // Container slot IDs: container.0 to container.35
            ...
            if (!empty($slotId)) {
                // ✅ Use item replace command with correct syntax:
                // item replace entity <target> <slot> with <item> [<count>]
                // Note: <item> format: minecraft:item_id or item_id
                //       <count> is optional, default is 1
                
                // Normalize item ID (ensure minecraft: prefix if not present)
                $normalizedItem = $item;
                if (!str_starts_with($normalizedItem, 'minecraft:')) {
                    $normalizedItem = 'minecraft:' . $normalizedItem;
                }
                
                // Build command with count (always specify count for clarity)
                $command = "item replace entity {$player} {$slotId} with {$normalizedItem} {$amount}";
                
                Log::info('MCPActionService: Sending item replace command', [
                    'server' => $server->uuid,
                    'player' => $player,
                    'command' => $command,
                    'item' => $item,
                ...
                try {
                    ...
                    // Clear cache to force refresh of player items
                    $worlds = ['world', 'world_nether', 'world_the_end'];
                    foreach ($worlds as $w) {
                        Cache::forget("server:{$server->uuid}:player_items:{$uuid}:{$w}");
                    }
                    Cache::forget("server:{$server->uuid}:fast_query");
                    
                    Log::info('MCPActionService: Item replace command sent successfully', [
                        'server' => $server->uuid,
                        'player' => $player,
                        'slotId' => $slotId,
                        'item' => $normalizedItem,
                    ...
                    // Fallback to regular give command if item replace fails
                    Log::warning('MCPActionService: Item replace failed, falling back to give command', [
                        'server' => $server->uuid,
                        'player' => $player,
                        'slot' => $slot,
                        'slotId' => $slotId,
                    ...
                    // Continue to use give command below (will place item in inventory automatically)
                }
            }
        }
        // Use 'give' instead of 'minecraft:give' for better compatibility with older versions
        $command = "give $player $item $amount";
        if ($metadata > 0) {
            $command .= " $metadata";
        }
        $this->commandRepository->setServer($server)->send($command);
        // Clear cache to force refresh of player items for all worlds
        // We need to clear cache for all possible worlds since we don't know which world the player is in
        $worlds = ['world', 'world_nether', 'world_the_end'];
        foreach ($worlds as $w) {
            Cache::forget("server:{$server->uuid}:player_items:{$uuid}:{$w}");
        }
        // Also clear fast_query cache to refresh player stats
        Cache::forget("server:{$server->uuid}:fast_query");
    }
    /**
    ...
    {
        // ✅ Use item replace command to remove item by replacing with air
        // Syntax: item replace entity <target> <slot> with minecraft:air
        ...
        if ($type === 'inventory') {
            ...
            if (isset($armorSlots[$slot])) {
                ...
                throw new DisplayException("Invalid armor slot: {$slot}");
            }
        } elseif ($type === 'offhand') {
            $slotId = 'weapon.offhand';
        } elseif ($type === 'ender_chest') {
            ...
            throw new DisplayException("Invalid item type: {$type}");
        }
        
        if (empty($slotId)) {
            throw new DisplayException("Invalid slot or type combination: slot {$slot}, type {$type}");
        }
        
        // Build command: item replace entity <target> <slot> with minecraft:air
        $command = "item replace entity {$name} {$slotId} with minecraft:air";
        
        Log::info('MCPActionService: Sending remove item command', [
            'server' => $server->uuid,
            'player' => $name,
            'command' => $command,
            'slot' => $slot,
        ...
        try {
            ...
            // ✅ Clear cache to force refresh of player items
            // Note: getPlayerItems uses UUID as cache key, but we only have player name here
            // Clear cache for all worlds (will be refreshed on next loadPlayerItems call)
            $worlds = ['world', 'world_nether', 'world_the_end'];
            foreach ($worlds as $w) {
                // Try to clear by name pattern (in case cache was set with name)
                Cache::forget("server:{$server->uuid}:player_items:{$name}:{$w}");
            }
            
            // Also clear fast_query cache which might contain player data
            Cache::forget("server:{$server->uuid}:fast_query");
            
            Log::info('MCPActionService: Remove item command sent successfully', [
                'server' => $server->uuid,
                'player' => $name,
                'slotId' => $slotId,
                'type' => $type,
            ...
            Log::error('MCPActionService: Failed to remove item', [
                'server' => $server->uuid,
                'player' => $name,
                'slot' => $slot,
                'slotId' => $slotId,
            ...
            throw new DisplayException("Failed to remove item: " . $e->getMessage());
        }
    }
    /**
     * Force save all player data (call save-all command).
     *
     * @param Server $server
     * @param bool $wait Wait for save to complete (default: true to ensure data is saved)
     * @return void
     */
    public function forceSavePlayerData(Server $server, bool $wait = true): void
    {
        try {
            // ✅ Send save-all command เพื่อ force save player data ทุกอย่าง
            // Note: Minecraft server จะ save player data ทันทีเมื่อได้รับ save-all command
            $this->commandRepository->setServer($server)->send('save-all');
            
            // ✅ รอเล็กน้อยเพื่อให้ server process command และ save player data
            // Minecraft server ต้องใช้เวลาเล็กน้อยในการ save player data files
            if ($wait) {
                usleep(200000); // Wait 0.2 seconds (200ms) for server to process save-all command and save files
            }
        } catch (\Exception $e) {
            // Log error but don't throw - allow getPlayerItems to continue
            Log::warning('MCPActionService: Failed to force save player data', [
                'server' => $server->uuid,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    {
        try {
            // ✅ Force save player data ก่อน refresh เพื่อให้ได้ข้อมูลล่าสุด
            // เรียก save-all และรอเล็กน้อยเพื่อให้ server save player data files ก่อนอ่าน
            $this->forceSavePlayerData($server, true);
            
            $playerDataPath = $world . '/playerdata/' . $uuid . '.dat';
            
            // ✅ Try to get player data file, catch DaemonConnectionException separately
            try {
                ...
                // Check HTTP status code to provide better error messages
                $statusCode = $e->getStatusCode();
                
                Log::warning('MCPActionService: DaemonConnectionException when getting player data file', [
                    'server' => $server->uuid,
                    'player_uuid' => $uuid,
                    'world' => $world,
                    'path' => $playerDataPath,
                ...
                if ($statusCode === 404) {
                    // File not found - try to find player data in other worlds
                    try {
                        $worlds = $this->getDetectedWorlds($server);
                        foreach ($worlds as $detectedWorld) {
                            $worldName = $detectedWorld['name'] ?? null;
                            if ($worldName && $worldName !== $world) {
                                try {
                                    $altPath = $worldName . '/playerdata/' . $uuid . '.dat';
                                    $playerData = $this->fileRepository->setServer($server)->getContent($altPath);
                                    // Found player data in another world - use it
                                    Log::info('MCPActionService: Found player data in different world', [
                                        'server' => $server->uuid,
                                        'player_uuid' => $uuid,
                                        'requested_world' => $world,
                                        'found_world' => $worldName,
                                    ]);
                                    // Continue processing with found world
                                    $world = $worldName;
                                    break;
                                } catch (\Pterodactyl\Exceptions\Http\Connection\DaemonConnectionException $altException) {
                                    // Not found in this world, continue searching
                                    continue;
                                }
                            }
                        }
                        
                        // If we still don't have player data, return error
                        if (!isset($playerData)) {
                            return ['error' => "Player data file not found in world '{$world}'. The player may need to join this world first or wait a moment for the file to be saved."];
                        }
                    } catch (\Exception $worldException) {
                        // Failed to detect worlds - return original error
                        return ['error' => "Player data file not found in world '{$world}'. The player may need to join this world first or wait a moment for the file to be saved."];
                    }
                } elseif ($statusCode >= 500) {
                    // Server error - Wings daemon connection error
                    // Don't include "code: 500" in error message - return user-friendly message
                    $requestId = $e->getRequestId();
                    if ($requestId) {
                        return ['error' => "Unable to connect to the server to retrieve player data. Please ensure the server is running and try again. (Request ID: {$requestId})"];
                    } else {
                        return ['error' => "Unable to connect to the server to retrieve player data. Please ensure the server is running and try again."];
                    }
                } else {
                    // Other HTTP errors (400-499) - extract error from response body if available
                    $previousException = $e->getPrevious();
                    if ($previousException && method_exists($previousException, 'getResponse')) {
                    ...
                    return ['error' => "An error occurred while retrieving player data. (HTTP {$statusCode})"];
                }
            }
            
            // ✅ Check if player data was successfully retrieved
            if (empty($playerData)) {
                // Try to find player data in other worlds
                try {
                    $worlds = $this->getDetectedWorlds($server);
                    foreach ($worlds as $detectedWorld) {
                        $worldName = $detectedWorld['name'] ?? null;
                    ...
                    // Failed to detect worlds - ignore
                }
            ...
            try {
                ...
                try {
                    ...
                    // Stats file not accessible - ignore, use empty stats
                }
            } catch (\Exception $e) {
                // Other errors - ignore, use empty stats
            }
            // ✅ Process inventory data - wrap in try-catch to handle any errors
            try {
                ...
                Log::warning('Failed to process inventory data', [
                    'server' => $server->uuid,
                    'player_uuid' => $uuid,
                    'world' => $world,
                    'error' => $e->getMessage(),
                ...
                return ['error' => 'Failed to process player data: ' . $e->getMessage()];
            }
            // ✅ Get player position - wrap in try-catch to handle any errors
            try {
                ...
                // Position not available - ignore, use empty position
                Log::debug('Failed to get player position', ['error' => $e->getMessage()]);
            }
            // ✅ Normalize items - wrap in try-catch to handle any errors
            try {
                ...
                Log::warning('Failed to normalize items', [
                    'server' => $server->uuid,
                    'player_uuid' => $uuid,
                    'world' => $world,
                    'error' => $e->getMessage(),
                ]);
                // Use empty arrays if normalization fails
                $inventoryData['inventory'] = $inventoryData['inventory'] ?? [];
                $inventoryData['ender_chest'] = $inventoryData['ender_chest'] ?? [];
                $inventoryData['offhand'] = $inventoryData['offhand'] ?? [];
                $inventoryData['armor'] = $inventoryData['armor'] ?? [];
            }
            // ✅ Get player avatar - wrap in try-catch to handle any errors
            try {
                $playerAvatar = $this->getPlayerAvatar('', $uuid);
            } catch (\Exception $e) {
                // Log avatar errors but don't fail - use default avatar
                Log::warning('Failed to get player avatar', [
                    'player_uuid' => $uuid,
                    'error' => $e->getMessage(),
                ]);
                $playerAvatar = '';
            ...
            // Wings daemon connection error - return user-friendly error message
            $statusCode = $e->getStatusCode();
            $requestId = $e->getRequestId();
            
            Log::warning('DaemonConnectionException in getPlayerItems', [
                'server' => $server->uuid,
                'player_uuid' => $uuid,
                'world' => $world,
                'status_code' => $statusCode,
            ...
            if ($statusCode === 404) {
                ...
                // Server error - return user-friendly message without "code: 500"
                if ($requestId) {
                    return ['error' => "Unable to connect to the server to retrieve player data. Please ensure the server is running and try again. (Request ID: {$requestId})"];
                } else {
                    return ['error' => "Unable to connect to the server to retrieve player data. Please ensure the server is running and try again."];
                ...
                // Other HTTP errors (400-499)
                return ['error' => "An error occurred while retrieving player data. (HTTP {$statusCode})"];
            }
        } catch (\Exception $e) {
            // Log all exceptions for debugging
            Log::error('Exception in getPlayerItems', [
                'server' => $server->uuid,
                'player_uuid' => $uuid,
                'world' => $world,
        ...
        if ($inventoryNode && method_exists($inventoryNode, 'getChildren') && $inventoryNode->getChildren()) {
            foreach ($inventoryNode->getChildren() as $itemNode) {
                try {
                    ...
                    // Skip invalid items - log for debugging
                    Log::debug('Failed to parse inventory item', ['error' => $e->getMessage()]);
                    continue;
                }
            }
        }
        if ($enderItemsNode && method_exists($enderItemsNode, 'getChildren') && $enderItemsNode->getChildren()) {
            foreach ($enderItemsNode->getChildren() as $itemNode) {
                try {
                    ...
                    // Skip invalid items - log for debugging
                    Log::debug('Failed to parse ender chest item', ['error' => $e->getMessage()]);
                    continue;
                }
            }
        }
        ...
        if ($equipmentNode && method_exists($equipmentNode, 'getChildren') && $equipmentNode->getChildren()) {
            ...
            if ($offhandNode) {
                try {
                    ...
                    Log::debug('Failed to parse offhand item', ['error' => $e->getMessage()]);
                }
            }
            $armorPieces = [
                'head' => 3,
            ...
            foreach ($armorPieces as $piece => $index) {
                ...
                if ($pieceNode) {
                    try {
                        ...
                        Log::debug('Failed to parse armor item', ['piece' => $piece, 'error' => $e->getMessage()]);
                    }
                }
            }
        } else {
            ...
            if ($handItemsNode && method_exists($handItemsNode, 'getChildren') && $handItemsNode->getChildren()) {
                ...
                if (isset($handItems[1])) {
                    try {
                        ...
                        Log::debug('Failed to parse hand item', ['error' => $e->getMessage()]);
                    }
                }
            }
                ...
                for ($i = 0; $i < 4; $i++) {
                    if (isset($armorItems[$i])) {
                        try {
                            ...
                            Log::debug('Failed to parse armor item from ArmorItems', ['index' => $i, 'error' => $e->getMessage()]);
                        }
                    }
                }
            }
    ...
    private function parseNbtItem($itemNode): array
    {
        try {
            ...
            $id = ($idNode && method_exists($idNode, 'getValue')) ? $idNode->getValue() : '';
            if (empty($id)) {
                $idNode = $itemNode->findChildByName('Id');
                $id = ($idNode && method_exists($idNode, 'getValue')) ? $idNode->getValue() : '';
            }
            ...
            $count = ($countNode && method_exists($countNode, 'getValue')) ? $countNode->getValue() : 1;
            if ($count === null || $count === 0) {
                $count = 1;
            }
            $slotNode = $itemNode->findChildByName('Slot') ?: $itemNode->findChildByName('slot');
            $slot = ($slotNode && method_exists($slotNode, 'getValue')) ? $slotNode->getValue() : null;
            $damageNode = $itemNode->findChildByName('Damage') ?: $itemNode->findChildByName('damage');
            $damage = ($damageNode && method_exists($damageNode, 'getValue')) ? $damageNode->getValue() : null;
        $item = [
        ...
        if ($damage !== null) {
            ...
            if ($maxDamageNode && method_exists($maxDamageNode, 'getValue')) {
            ...
            if ($tagNode && method_exists($tagNode, 'getChildren')) {
                ...
                if ($enchNode && method_exists($enchNode, 'getChildren')) {
                    ...
                    foreach ($enchNode->getChildren() as $enchChild) {
                        try {
                            ...
                            if ($enchId && $enchLvl && method_exists($enchId, 'getValue') && method_exists($enchLvl, 'getValue')) {
                                $item['enchantments'][] = [
                                    'id' => $enchId->getValue(),
                ...
                if ($displayNode && method_exists($displayNode, 'getChildren')) {
                    ...
                    if ($nameNode && method_exists($nameNode, 'getValue')) {
        ...
        return $item;
            ...
            // Return empty item if parsing fails
            Log::debug('Failed to parse NBT item', ['error' => $e->getMessage()]);
            return ['id' => '', 'count' => 0];
        }
    }
    /**
    ...
    public function getDetectedWorlds(Server $server): array
    {
        try {
            ...
            try {
                ...
                Log::error('MCPActionService: DaemonConnectionException when getting root directory', [
                    'server' => $server->uuid,
                    'error' => $e->getMessage(),
                    'status_code' => $e->getStatusCode(),
                ]);
                // Return empty array if cannot connect to daemon
                return [];
            } catch (\Exception $e) {
                Log::error('MCPActionService: Exception when getting root directory', [
                    'server' => $server->uuid,
                    'error' => $e->getMessage(),
                    'trace' => $e->getTraceAsString(),
                ]);
            ...
            // Log the raw response from getDirectory
            Log::info('MCPActionService: Raw directory response', [
                'server' => $server->uuid,
                'files_structure' => $files,
                'has_contents' => isset($files['contents']),
                'contents_type' => isset($files['contents']) ? gettype($files['contents']) : 'not set',
                'contents_count' => isset($files['contents']) && is_array($files['contents']) ? count($files['contents']) : 0,
            ]);
            
            if (!isset($files['contents']) || !is_array($files['contents'])) {
                Log::warning('MCPActionService: Invalid directory structure returned', [
                    'server' => $server->uuid,
                    'files' => $files,
                    'files_keys' => is_array($files) ? array_keys($files) : 'not array',
                ]);
                return [];
            }
            
            $worlds = [];
            Log::info('MCPActionService: Starting world detection', [
                'server' => $server->uuid,
                'total_files' => count($files['contents']),
                'file_names' => array_map(function($file) {
                    return $file['name'] ?? 'no name';
                }, array_slice($files['contents'], 0, 10)), // Log first 10 file names
            ]);
            
            foreach ($files['contents'] as $file) {
                if (!isset($file['directory']) || !$file['directory']) {
                    continue;
                }
                ...
                $excludedDirs = ['.', '..', 'plugins', 'logs', 'libraries', 'crash-reports', 'mods', 'config', 'resourcepacks', 'saves', 'versions', '.cache'];
                if (in_array($fileName, $excludedDirs)) {
                    Log::debug('MCPActionService: Skipping excluded directory', [
                        'server' => $server->uuid,
                        'directory' => $fileName,
                    ]);
                    continue;
                ...
                // Log each directory we're checking
                Log::info('MCPActionService: Checking directory for world', [
                    'server' => $server->uuid,
                    'directory' => $fileName,
                ]);
                
                try {
                    // Try to read level.dat file (handle spaces in folder names)
                    $levelDatPath = "{$fileName}/level.dat";
                    Log::debug('MCPActionService: Attempting to read level.dat', [
                        'server' => $server->uuid,
                        'path' => $levelDatPath,
                    ]);
                    
                    $levelDat = $this->fileRepository->setServer($server)->getContent($levelDatPath);
                    if ($levelDat) {
                        $hasPlayerData = false;
                        try {
                            $playerDataDir = $this->fileRepository->setServer($server)->getDirectory("{$fileName}/playerdata");
                            $hasPlayerData = !empty($playerDataDir['contents']) && is_array($playerDataDir['contents']) && count($playerDataDir['contents']) > 0;
                        } catch (\Exception $e) {
                            Log::debug('MCPActionService: Could not access playerdata directory', [
                                'server' => $server->uuid,
                                'world' => $fileName,
                                'error' => $e->getMessage(),
                            ]);
                        ...
                        $hasStats = false;
                        try {
                            $statsDir = $this->fileRepository->setServer($server)->getDirectory("{$fileName}/stats");
                            $hasStats = !empty($statsDir['contents']) && is_array($statsDir['contents']) && count($statsDir['contents']) > 0;
                        } catch (\Exception $e) {
                            Log::debug('MCPActionService: Could not access stats directory', [
                                'server' => $server->uuid,
                                'world' => $fileName,
                                'error' => $e->getMessage(),
                            ]);
                        ...
                        $worlds[] = [
                            'name' => $fileName,
                            'display_name' => $this->formatWorldName($fileName),
                            'has_player_data' => $hasPlayerData,
                            'has_stats' => $hasStats,
                        ];
                        
                        Log::info('MCPActionService: Detected world', [
                            'server' => $server->uuid,
                            'world' => $fileName,
                            'has_player_data' => $hasPlayerData,
                            'has_stats' => $hasStats,
                    ...
                    // Cannot read level.dat - not a world or access denied
                    Log::debug('MCPActionService: Cannot read level.dat for directory', [
                        'server' => $server->uuid,
                        'directory' => $fileName,
                        'error' => $e->getMessage(),
                        'status_code' => $e->getStatusCode(),
                    ...
                    Log::debug('MCPActionService: Error checking directory', [
                        'server' => $server->uuid,
                        'directory' => $fileName,
                        'error' => $e->getMessage(),
                    ]);
            ...
            Log::info('MCPActionService: Detected worlds summary', [
                'server' => $server->uuid,
                'world_count' => count($worlds),
                'worlds' => array_column($worlds, 'name'),
            ]);
            
            return $worlds;
        } catch (\Exception $e) {
            Log::error('MCPActionService: Failed to detect worlds', [
                'server' => $server->uuid,
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);
