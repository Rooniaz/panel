                    ...
                    {({ submitForm }) => (
                        <div className="flex flex-col space-y-4">
                            {showWarning && (
                                <Alert type="warning" className="mb-2">
                                    <div className="flex items-center">
                                        <p>
                                            Version changes can affect world data compatibility.
                                        </p>
                                    </div>
                                </Alert>
                            )}
                                ...
                                <div>
                                    <div className="text-sm text-neutral-300">
                                        ...
                                        <div css={tw`bg-neutral-800 rounded p-4`}>
                                            <h3 className="text-md font-bold text-neutral-100">{versionDetails.update_title || versionDetails.version_number}</h3>
                                            <p className="mb-2">
                                                Server Version: <u>{versionDetails.server_version === 'N/A' ? 'Not yet released ' : versionDetails.server_version}</u>
                                                <Tooltip placement="top" content={versionDetails.server_version === 'N/A' ?
                                                    'Server is not available yet for this version' :
                                                    'Server is available for this version'
                                                }>
                                                    <span>
                                                        {versionDetails.server_version === 'N/A' ? (
                                                            <FontAwesomeIcon icon={faExclamationCircle} className="ml-1 text-red-400" />
                                                        ) : (
                                                            <FontAwesomeIcon icon={faCheckCircle} className="ml-1 text-green-400" />
                                                        )}
                                                    </span>
                                                </Tooltip>
                                            </p>
                            ...
                            <Dialog.Footer>
                                ...
                                >
                                    <FontAwesomeIcon icon={isSubmitting ? faSpinner : faDownload} spin={isSubmitting} css={tw`mr-1`} />
                                    Install
                                </Button>
                            </Dialog.Footer>
                        </div>
