...
class MinecraftVersionController extends ClientApiController
{
    ...
    public function updateMinecraftVersion(Request $request, Server $server): array
    {
        ...
        $fileRepository = app()->make(\Pterodactyl\Repositories\Wings\DaemonFileRepository::class);
        $powerRepository = app()->make(\Pterodactyl\Repositories\Wings\DaemonPowerRepository::class);
    ...
    private function getServerJarVariableId(Server $server): int
    {
        $variable = $server->variables()->where('env_variable', 'SERVER_JARFILE')->first();
        
        if (!$variable) {
            throw new \Exception('Could not find SERVER_JARFILE variable for this server.');
        }
        
        return $variable->id;
    }

    private function isZipFile(string $url): bool
    {
        ...
        try {
            ...
            if (isset($headers['Content-Type'])) {
                ...
                return strpos($contentType, 'application/zip') !== false || 
                       strpos($contentType, 'application/x-zip-compressed') !== false;
            }
        } catch (\Exception $e) {
        }
