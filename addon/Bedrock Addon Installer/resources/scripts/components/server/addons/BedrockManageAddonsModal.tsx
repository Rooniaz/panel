...
const AddonIconContainer = ({ type, children }: AddonIconContainerProps) => (
    <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${type === 'behavior' ? 'bg-red-500/20' : 'bg-blue-500/20'}`}>
        {children}
    </div>
);
...
const TabButton = ({ active, children, onClick }: TabButtonProps) => (
    <button
        className={`px-4 py-2 text-sm transition-colors duration-150 ${active
            ? 'border-b-2 border-primary-500 text-neutral-200'
...
const AddonVersion = ({ children }: ComponentProps) => (
    <span className="text-xs text-neutral-400">
        {children}
    </span>
);
const AddonActions = ({ children }: ComponentProps) => (
...
const WorldBadge = ({ isDefault, children }: WorldBadgeProps) => (
    <span
        className={`text-xs px-2 py-1 rounded ${isDefault
...
const DragHandle = ({ children }: ComponentProps) => (
    <div className="cursor-move text-neutral-400 hover:text-neutral-200 mr-3">
        {children}
    </div>
);
                                                ...
                                                <AddonTitle>
                                                    <AddonIconContainer type="behavior">
                                                        ...
                                                        <div className={`${pack.has_icon ? 'hidden' : ''}`}>
                                                            <FontAwesomeIcon icon={faCode} className="w-5 h-5 text-orange-400" />
                                                        </div>
                                                    </AddonIconContainer>
                                                    <div className="flex-1 min-w-0">
                                                        <div className="font-medium truncate">{pack.name}</div>
                                                        <AddonVersion>v{Array.isArray(pack.version) ? pack.version.join('.') : pack.version}</AddonVersion>
                                                    </div>
                                                </AddonTitle>
                                                <AddonActions>
                                                    <Button.Text
                                                        size={"small" as any}
                                                        variant={"text" as any}
                                                        onClick={() => moveAddon('behavior', index, Math.max(0, index - 1))}
                                                        disabled={index === 0}
                                                    >
                                                    ...
                                                    <Button.Text
                                                        size={"small" as any}
                                                        variant={"text" as any}
                                                        onClick={() => moveAddon('behavior', index, Math.min(behaviorPacks.length - 1, index + 1))}
                                            ...
                                            <AddonHeader>
                                                <AddonTitle>
                                                    <AddonIconContainer type="resource">
                                                        ...
                                                        <div className={`${pack.has_icon ? 'hidden' : ''}`}>
                                                            <FontAwesomeIcon icon={faPalette} className="w-5 h-5 text-blue-400" />
                                                        </div>
                                                    </AddonIconContainer>
                                                    <div className="flex-1 min-w-0">
                                                        <div className="font-medium truncate">{pack.name}</div>
                                                ...
                                                <AddonActions>
                                                    <Button.Text
                                                        size={"small" as any}
                                                        variant={"text" as any}
                                                        onClick={() => moveAddon('resource', index, Math.max(0, index - 1))}
                                                        disabled={index === 0}
                                                    >
                                                        <FontAwesomeIcon icon={faArrowUp} />
                                                    </Button.Text>
                                                    <Button.Text
                                                        size={"small" as any}
                                                        variant={"text" as any}
                                                        onClick={() => moveAddon('resource', index, Math.min(resourcePacks.length - 1, index + 1))}
